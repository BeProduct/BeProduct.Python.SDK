...
class TestUploadMixin(unittest.TestCase):
    ...
    def test_attributes_upload(self):
        ...
        upload_id = self.client.style.attributes_upload(
            header_id=tmp_style["id"], filepath=self.image_path
        )
        ...
        self.assertTrue(fname and fname in style["headerData"]["frontImage"]["origin"])

        # position upload
        upload_id = self.client.style.attributes_upload(
            header_id=tmp_style["id"], filepath=self.image_path, position="back"
        )

        self.assertTrue(test_helpers.check_upload_status(self, upload_id))

        style = self.client.style.attributes_get(header_id=tmp_style["id"])
        self.assertTrue(fname and fname in style["headerData"]["backImage"]["origin"])
    ...
    def test_app_list_upload(self):
        ...
        upload_id = self.client.style.app_list_upload(
            header_id=tmp_style["id"],
            app_id=self.config.LIST_APP["id"],
        ...
        self.assertTrue(len(item) == 1)
        self.assertTrue(fname and fname in item[0]["origin"])
    ...
    def test_app_attachments_upload(self):
        ...
        self.assertTrue(fname and fname in app["data"]["files"][0]["url"])
    ...
    def test_app_imageform_upload(self):
        ...
        self.assertTrue(fname and fname in app["data"]["image"][0]["origin"])
