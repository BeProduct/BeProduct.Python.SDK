...
class TestUploadMixin(unittest.TestCase):
    ...
    def test_attributes_upload(self):
        ...
        self.assertTrue(
            style['headerData']['frontImage']['origin'].endswith(fname))
        ...
        style = self.client.style.attributes_get(header_id=tmp_style['id'])
        self.assertTrue(
            style['headerData']['backImage']['origin'].endswith(fname))
    ...
    def test_app_list_upload(self):
        ...
        def get_app():
            return self.client.style.app_get(header_id=tmp_style['id'],
                                             app_id=self.config.LIST_APP['id'])

        # item insert
        self.client.style.app_list_update(header_id=tmp_style['id'],
                                          app_id=self.config.LIST_APP['id'],
                                          list_items=[{
                                              'itemId':
                                              item_id,
                                              'itemFields':
                                              self.config.LIST_APP_ITEM_INSERT
                                          }])
    ...
    def test_app_imageform_upload(self):
        ...
        app = self.client.style.app_get(header_id=tmp_style['id'],
                                        app_id=self.config.IMAGEFORM_APP['id'])
