"""
File: _common_apps_async.py
Author: Yuri Golub
Email: yuri.golub@beproduct.com
Github: https://github.com/BeProduct
Description: Apps Mixin for every master folder - Async Version
"""
...
class AppsMixinAsync:
    """
    Apps mixin class for every master folder - Async Version
    """
    ...
    async def app_schema(self, app_id: str):
    ...
    async def __app_list(self, bb: __BlackBox, folder_id: str):
        logging.debug("APP_LIST: Cache miss. Fetching fresh data.")
        return await self.client.raw_api.get(
            f"{self.master_folder}/Pages?headerId={bb.header_id}"
        )

    async def app_list(self, header_id: str, folder_id: str = None):
        ...
        if folder_id:
            ...
            return await self.__app_list(self.__BlackBox(header_id=header_id), folder_id)
        else:
            logging.debug(
    ...
    async def app_get(
        ...
        if not app_id:
            apps = await self.app_list(header_id, folder_id)
            for app in apps:
    ...
    async def app_form_update(self, header_id: str, app_id: str, fields):
    ...
    async def app_grid_update(self, header_id: str, app_id: str, rows):
        """Updates form application
        :header_id: ID of the style, material, etc
        :app_id: ID of the application / page
    ...
    async def app_list_update(self, header_id: str, app_id: str, list_items):
    ...
    async def app_attachments_delete(self, header_id: str, app_id: str, filenames_to_remove):
